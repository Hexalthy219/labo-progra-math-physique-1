...
def dist_terrain_vers_fenetre(dist_t, largeur_fenetre, largeur_terrain):
    dist_f = dist_t*largeur_fenetre/largeur_terrain
    return dist_f
...
def coord_terrain_vers_fenetre(point_terrain, dimensions_fenetre,
    ...
    largeur_fenetre, hauteur_fenetre = dimensions_fenetre
    x_f = dist_terrain_vers_fenetre(x_t, largeur_fenetre, largeur_terrain)
    y_f = dist_terrain_vers_fenetre(y_t, largeur_fenetre, largeur_terrain)
    x_f+= (largeur_fenetre/2)
    y_f = -y_f
    y_f+=(hauteur_fenetre/2)
    return (x_f, y_f)
...
def coord_fenetre_vers_terrain(point_fenetre, dimensions_fenetre, largeur_terrain):
    x_f, y_f = point_fenetre
    largeur_fenetre, hauteur_fenetre = dimensions_fenetre
    x_t = x_f-(largeur_fenetre/2)
    y_t = y_f-(hauteur_fenetre/2)
    y_t = -y_t
    y_t = dist_fenetre_vers_terrain(y_t, largeur_fenetre, largeur_terrain)
    x_t = dist_fenetre_vers_terrain(x_t, largeur_fenetre, largeur_terrain)
    return (x_t, y_t)
