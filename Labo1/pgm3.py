...
def calculer_vitesse_acceleration(altitude_avion, temps_maintenant):
    global v_y_1, v_y_2, a_y, temps_precedent, altitude_precedente
    v_y_2 = v_y_1
    v_y_1 = -(altitude_avion - altitude_precedente)/(temps_maintenant - temps_precedent)
    a_y = (v_y_1 - v_y_2)/(temps_maintenant - temps_precedent)
    altitude_precedente = altitude_avion
...
def mrua_1d(depart, temps_depart, acceleration, temps_maintenant):
    p = depart + acceleration*(temps_maintenant - temps_depart)*(temps_maintenant - temps_depart)/2
    return p

# *** A MODIFIER *********************************************************

def calculer_tir(altitude_larguage, altitude_cible, acceleration, prochain_temps_cible, temps_maintenant):
    duree_chute=math.sqrt(2*(altitude_cible-altitude_larguage)/acceleration)
    temps_tir = prochain_temps_cible - duree_chute
