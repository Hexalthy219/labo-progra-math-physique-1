...
def mua_2d(depart, temps_depart, vitesse_initiale, acceleration_verticale, temps_maintenant):
    delta_t = temps_maintenant - temps_depart
    x_t = depart[0] + vitesse_initiale[0] * delta_t
    y_t = depart[1] + vitesse_initiale[1] * delta_t + acceleration_verticale * (delta_t**2)/2
    return (x_t, y_t)

# *** A MODIFIER *********************************************************

def calculer_vitesse(depart, angle, cible, acceleration_verticale):
    delta_x = cible[0] - depart[0]
    delta_y = cible[1] - depart[1]
    v0 = 0
    v0i = ((acceleration_verticale * (delta_x**2))/(2 * (delta_y - (math.tan(angle) * delta_x)) * ((math.cos(angle)))**2))
    if  v0i >= 0:
        v0 = math.sqrt(v0i)
        tir_est_possible = True
    else:
        tir_est_possible = False
    return (tir_est_possible, v0)

# *** A MODIFIER *********************************************************

def calculer_impact(depart, angle, hauteur_sol, vitesse, acceleration_verticale):
    delta_y = hauteur_sol - depart[1]
    racine = math.sqrt((math.tan(angle))**2 + (2 * acceleration_verticale * delta_y / (vitesse**2 * math.cos(angle)**2)))
    x1 = (-math.tan(angle) + racine) * (vitesse**2 * math.cos(angle)**2 / acceleration_verticale)
    x2 = (-math.tan(angle) - racine) * ((vitesse**2 * math.cos(angle)**2) / acceleration_verticale)
    x=0
    if x1 >= x2:
        x = x1
    else:
...
dimensions_fenetre = (3000, 1800)  # en pixels
images_par_seconde = 25

couleur_fond = BLEUCIEL
position_canon = (50, 1650)
