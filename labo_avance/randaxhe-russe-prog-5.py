...
FENETRE_HAUTEUR = 600
dimension_fenetre_Julia = (1600, 600)
dimension_fenetre_Mandelbrot = (800, 600)
...
def complexe_vers_pixel(z):
    ...
    z_a = int(z_a * pixel_par_complex + 800/2)
    z_b = int(-z_b *pixel_par_complex + 600/2)
    return(z_a, z_b)
...
def pixel_vers_complexe(z):
    ...
    z_a = (z_a - 800/2) * complexe_par_pixel
    z_b = -(z_b - 600/2) * complexe_par_pixel
    return (z_a, z_b)
...
def test_convergence(c, z0, nbr_it):
    global d
    z = z0
    d = 0
    ...
    for evenement in pygame.event.get():
        ...
        if evenement.type == pygame.MOUSEBUTTONDOWN:
            if evenement.button == 1:
                ...
                nouveau_clic = True
                pos_click = pixel_vers_complexe(pos_click)
    return

pygame.init()

fenetre = pygame.display.set_mode(dimension_fenetre, 0)
pygame.display.set_caption("Pgm5 fractal")

pixels = pygame.PixelArray(fenetre)
fenetre.fill(BLANC)
pygame.display.flip()

for i in range(dimension_fenetre_Mandelbrot[0]):
    for j in range(dimension_fenetre_Mandelbrot[1]):
        z1 = [i,j]
        z1 = pixel_vers_complexe(z1)

        if test_convergence(z1, [0,0], 200):
            pixels[i][j] = NOIR
        else:
            pixels[i][j] = couleur(d + 1)

    pygame.display.update(((i, 0), (1, dimension_fenetre_Mandelbrot[1])))
    traiter_evenement()
...
while True:
    traiter_evenement()
    if nouveau_clic:
        for i in range(dimension_fenetre_Mandelbrot[0], dimension_fenetre_Julia[0]):
            for j in range(dimension_fenetre_Julia[1]):
                z1 = [i - 800,j]
                z1 = pixel_vers_complexe(z1)
                if test_convergence(pos_click, z1, 1000):
                    pixels[i][j] = NOIR
                else:
                    pixels[i][j] = couleur(d + 1)

            pygame.display.update(((i, 0), (1, dimension_fenetre_Julia[1])))
        nouveau_clic = False
    pygame.time.wait(25)
